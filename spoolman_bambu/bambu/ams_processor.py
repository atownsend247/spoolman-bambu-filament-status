...
def tray_validator(tray) -> bool:
    ...
    if tray["remain"] <= 0 or tray["tray_uuid"] == "00000000000000000000000000000000" or tray["tray_color"] == "00000000":
    ...
    if amsId not in last_ams_data.keys() or (amsId in last_ams_data.keys() and last_ams_data[amsId] != ams_data):
        ...
        for tray in ams_data["tray"]:
            ...
            if tray_validator(tray):
                ...
                logger.info(f"{processing_empty_prefix}  {amsId}{trayId} {tray['tray_sub_brands']} {tray['tray_color']} ({tray['remain']}%) [[{tray['tray_uuid']}]]...")
                ...
                internal_filament_matches = check_spool_matches_internal(spoolman_internal_filaments, external_filament_match, tray)
                    ...
                    if len(internal_filament_matches) > 0:
                        ...
                        if internal_filament_matches[0]["external_id"] == spool["filament"]["external_id"]:
                            ...
                            if spoolman_custom_tag_api not in spool["extra"].keys():
                                logger.info(f"{processing_empty_prefix}  - Checking tags IF unclaimed: {spool['extra']}")
                            ...
                            elif spoolman_custom_tag_api in spool["extra"].keys() and spool["extra"][spoolman_custom_tag_api] == "\"\"":
                                logger.info(f"{processing_empty_prefix}  - Checking tags ELIF unclaimed: {spool['extra']}")
                                # spoolman_spools[spool_index] = update_existing_spool(spool, tray, time)
                                unclaimed_spool_found_index = spool_index
                                unclaimed_spool_found = spool
                                active_spool_found = True
                            # if matching spool has tag set
                            elif spoolman_custom_tag_api in spool["extra"].keys() and spool["extra"][spoolman_custom_tag_api] == f"\"{tray['tray_uuid']}\"":
                                logger.info(f"{processing_empty_prefix}  - Checking tags ELIF claimed: {spool['extra']}")
                                # spoolman_spools[spool_index] = update_existing_spool(spool, tray, time)
                ...
                if not active_spool_found:
                    if len(internal_filament_matches) == 0:
                        ...
                        new_spool = create_new_filament_and_spool(
                            external_filament_match,
                            tray,
                            printer_id,
                            time
                        )
                        ...
                        new_spool = create_new_spool(
                            internal_filament_matches[0],
                            tray,
                            printer_id,
                            time
                        )
...
def create_new_spool(filament, tray, printer_id, current_time):
    ...
    new_spool_data = {
        ...
        "location": printer_id,
        "extra": {
            f"{spoolman_custom_tag_api}": f"\"{tray['tray_uuid']}\""
        }
    }
...
def update_existing_spool(spool, tray, printer_id, current_time):
    ...
    if spool["remaining_weight"] != remaining_weight:
        logger.info(f"{processing_empty_prefix}  - Patch spool {spool['id']} weight:{remaining_weight}, {spoolman_custom_tag}:{tray['tray_uuid']}, last:{current_time}")
        spool_patch_data = {
        ...
        updated_spool = spoolman_instance.patch_spool(
            spool["id"],
            spool_patch_data
        )
